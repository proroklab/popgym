...
class LastActionWrapper(Wrapper):
    ...
    def __init__(self, env: Env, null_action: Optional[ActType] = None):
        super().__init__(env)
        self.observation_space = LastActionWrapper.add_act_space_to_obs_space(
    ...
    def add_act_space_to_obs_space(
        ...
        if isinstance(
            ...
            if set(observation_space.keys()) == {OBS, STATE}:
                ...
                observation_space[OBS] = LastActionWrapper.add_act_space_to_obs_space(
    ...
    def add_act_to_obs(
        ...
        if isinstance(
            ...
            if set(observation_space.keys()) == {OBS, STATE}:
                # Observation comes from ObservabilityWrapper with
                # observability level FULL_AND_PARTIAL
                obs[OBS] = LastActionWrapper.add_act_to_obs(
    ...
    def get_null_action(action_space: spaces.Space) -> ActType:
        ...
        if isinstance(
            ...
            action = tuple(
                LastActionWrapper.get_null_action(action_space_)
                for action_space_ in action_space
            )
        elif isinstance(action_space, spaces.Dict):
            action = {
                key: LastActionWrapper.get_null_action(value)
    ...
    def reset(self, **kwargs):
        if kwargs.get("return_info", False):
            ...
            obs = LastActionWrapper.add_act_to_obs(
