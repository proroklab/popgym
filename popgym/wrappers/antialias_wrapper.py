...
class AntialiasWrapper(Wrapper):
    """Wrapper that undoes aliasing produces by the LastActionWrapper
    ...
    def __init__(self, env: Env):
        super().__init__(env)
        self.observation_space = AntialiasWrapper.antialias_obs_space(
    ...
    def antialias_obs_space(observation_space: spaces.Space) -> spaces.Space:
        ...
        if isinstance(
            ...
            if set(observation_space.keys()) == {OBS, STATE}:
                ...
                observation_space[OBS] = AntialiasWrapper.antialias_obs_space(
    ...
    def antialias_obs(
        ...
        if isinstance(
            ...
            if set(observation_space.keys()) == {OBS, STATE}:
                # Observation comes from ObservabilityWrapper
                # with observability level FULL_AND_PARTIAL
                obs[OBS] = AntialiasWrapper.antialias_obs(
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, dict]:
        ...
        self._tstep += 1
        obs = AntialiasWrapper.antialias_obs(
            self.env.observation_space, obs, self._tstep
        )
